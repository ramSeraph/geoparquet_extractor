...
async function init(dbPath) {
  const wasmUrl = new URL(/* @vite-ignore */ 'wa-sqlite-async.wasm', import.meta.url).href;
...
// --- Utility: fileToAsyncBuffer for OPFS File objects ---
...
async function writeFromParquet({ parquetFileName, gpkgFileName }, msgId) {
  ...
  // Pass 1: collect metadata (geometry types, bbox)
  ...
  try {
    await exec('PRAGMA application_id = 0x47503130');
    await exec('PRAGMA user_version = 10400');
    ...
    // Pass 2: read one row-group at a time, insert into SQLite
